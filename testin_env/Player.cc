...
void Player::loadDeck(string filename){
	...
	while(getline(file, cardName)){
			...
			if(type == "Minion"){
					...
					//int cost, int def, int atk, Trigger tr, Effect *trigger = nullptr,
					//Effect *actEff = nullptr, int ab_cost = 0, Info* summon_info = nullptr);
					...
					if (triggertype == "Null"){
							...
							card >> atk_c >> def_c >> act_c >> ab_cost >> kill >> unsummon >> resurrect >> player >> playermagic >> single >> zone >> charge;
							//card.ignore();
							//cout << atk_c << def_c << act_c << ab_cost << kill << unsummon << resurrect << player << playermagic << single << zone << charge;
							Effect* eff = new Effect{atk_c,def_c,act_c,ab_cost,kill,unsummon,resurrect,player,playermagic,single,zone,charge};
					...
					//const std::string &description, int cost, bool multiplier = false, int act_round = 0);
					...
					//Spell(const Player *sec, const std::string &name, const Effect *eff,
